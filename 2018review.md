...
最早在 15 年接触了可视化，当时做了一个在线家装系统、一个在线 3D 模型平台。

家装系统对 JS 设计能力要求比较高，需要建立房间、墙壁、窗户、各种物体的模型。此外就是当时很陌生的 3D 技术。
刚开始从 THREEJS 入手逐步了解了 3D 里面的几何、坐标系、镜头、光线、材质、碰撞、着色器、GPU 管线、光的原理、眼球的构造，我们还体验了 VR 系统，通过 nodejs 做 api 侦测手势在房间行走、查看。

在线 3D 模型我们主要做精细的模型展示。例如把房间的效果金属效果、玻璃效果、镜子效果、光线效果做好，一进入那样的房间，都令人感动。

现在我们的项目要求的是大数据的展示效果、更多的是各种图表展示、和地球这样的效果。echart、d3、稍微的
threejs。

考虑到可视化的需求会越来越多我们考虑开发一套 DataV 这样的系统，可以快速拖拽生成大屏系统。
...
这样一套平台组件是支撑，只有丰富的组件才能彰显平台的价值。其实这相当于可视化编程语言，只不过语言的基本单元是一个个可视化组件。

组件规范影响着这个平台的能力，类似编程语言里面的一些逻辑定义。

数据规范定义了这个平台怎么接入数据是这个平台的芯。

APP 编程系统主要来编辑和存储可视化系统。

这个系统我们现在还是个半成品，后面要满足业务对产品的所以需求。

> 理解产品需求、梳理产品模型关系到了这样一个产品的成败

## CLI

之前我们视图使用业界流行的一些技术方案开发资金的 cli 来完成 vue-cli 类似的功能，通过 git submodule 来支撑微服务的开发。开发了三个月后实现了基本的雏形可以用来进行开发和构建。

后面使用过程中除了功能不全，又踩了很多坑。既然业界的 vue-cli 和 lerna 这两个工具很成熟，我们就本着 kiss 原则，基于他们封装了自己的一个 CLI。这个 CLI 融合了 vue-cli 和 lerna 的优势。

其他团队也遇到了开发 monorepo 的需求，业界也没找到很好的解决方案。后面我们会整理推出自己的 CLI。
...
我们基于业界优秀的组件库，开发了一套自己的组件库。主要目的是可以积累和完善出适合我们场景的基础组件库。

### 业务组件库

针对业务我们持续抽象剥离、我们抽象出了常用的业务组件库。业务组件库是对基础组件库的封装、业务组件库适应
特定的产品需求。当做业务的时候，我们可以快速构建。

后面我们会持续规范、标准业务组件库，设计好业务组件库的目标和范围、标准。当积累足够的业务组件的时候，基于
业务组件可以产品很多模板，再基于这些模板来快速开发。
